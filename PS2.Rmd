...
* $p$ = the probability of a Canadian rating their overall satisfaction above the sample average. We call the LHS the “log odds”. This is modeled by Eq 1,
* $K$ = (number of predictors)x(number of levels in each predictor)
...
We utilize stepwise variable selection when formualting our model. Although removing the variables does result in lower BIC, it does not necessarily mean that the removed variables do not affect the response.
Scores can also vary over the years for various reasons that the survey does not capture. In a year like 2020, other factors will definitely affect satisfaction in life more thus we cannot generalize this analysis to the real world.
Finally, every predictor in our model is statistically significant, which is not neccessarily a good thing

It is also important to note the topic of satisfaction in life is subjective, and the variables selected may not accurately portray an individuals factors in regards to satisfaction.
